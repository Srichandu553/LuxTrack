...
function AccountPage({ onBack, onHome, onLogout }) {
    ...
    const saveProfile = async (event) => {
        ...
        try {
            ...
            setUser(result.user); setName(result.user.name || ""); setEditingProfile(false); setMessage(result.message);
        } catch (requestError) {
        ...
        <main className="account-page">
            ...
            <div className="account-layout">
                ...
                <section className="account-content">
                    {activeSection === "profile" ? <form className="account-panel" onSubmit={saveProfile}><div className="panel-heading"><div><span>PROFILE DETAILS</span><h2>Your identity</h2><p>Keep your account information current.</p></div><div className="panel-heading-actions">{!editingProfile && <button type="button" className="account-edit-button" onClick={() => { clearFeedback(); setEditingProfile(true); }}>Edit profile</button>}<div className="panel-icon">✦</div></div></div><label>Display name<input value={name} disabled={!editingProfile} onChange={(event) => setName(event.target.value)} minLength={2} maxLength={80} required /></label><label>Email address<input value={user?.email || ""} readOnly disabled /></label><p className="field-note">Your email is used for secure sign-in and notifications.</p>{error && <p className="account-error">{error}</p>}{message && <p className="account-message">{message}</p>}{editingProfile && <div className="account-form-actions"><button className="account-submit" disabled={saving} type="submit">{saving ? "Saving…" : "Save profile"}</button><button type="button" className="account-cancel" onClick={() => { setName(user?.name || ""); setEditingProfile(false); clearFeedback(); }}>Cancel</button></div>}</form> : <form className="account-panel" onSubmit={submitPassword}><div className="panel-heading"><div><span>SECURITY</span><h2>Protect your account</h2><p>Use a unique password of at least six characters.</p></div><div className="panel-icon">⌁</div></div><label>Current password<input type="password" minLength={6} value={currentPassword} onChange={(event) => setCurrentPassword(event.target.value)} required /></label><label>New password<input type="password" minLength={6} value={newPassword} onChange={(event) => setNewPassword(event.target.value)} required /></label>{error && <p className="account-error">{error}</p>}{message && <p className="account-message">{message}</p>}<button className="account-submit" disabled={saving} type="submit">{saving ? "Updating…" : "Update password"}</button></form>}
